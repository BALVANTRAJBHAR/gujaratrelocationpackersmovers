...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!propertyId || !rawPath) return jsonResponse({ error: 'property_id and raw_path required' }, 400);

    const user = await getAuthedUser(supabaseUrl, anonKey, jwt);

    const [prop] = await getRest<PropertyRow[]>(
      `${supabaseUrl}/rest/v1/properties?id=eq.${propertyId}&select=id,owner_user_id`,
      serviceKey
    );

    if (!prop) return jsonResponse({ error: 'Property not found' }, 404);
    if (!prop.owner_user_id || String(prop.owner_user_id) !== user.id) return jsonResponse({ error: 'Forbidden' }, 403);
    ...
    if (kind === 'photo') {
      ...
      const finalPath = `properties/${propertyId}/photo-${randomHex(12)}.jpg`;
      ...
      const [row] = await postRest(`${supabaseUrl}/rest/v1/property_uploads`, serviceKey, {
    ...
    if (kind === 'video') {
      ...
      const finalPath = `properties/${propertyId}/video-${randomHex(12)}.mp4`;
