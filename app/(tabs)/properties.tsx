  ...
  return (
    <View style={{ flex: 1, backgroundColor: pageBg }}>
      <YStack backgroundColor={theme.headerBg} padding={16} paddingTop={18} borderBottomWidth={1} borderBottomColor={border}>
        <Text color={titleColor} fontSize={18} fontWeight="900">Properties</Text>
        <Text color={muted} fontSize={12} fontWeight="600">Manage your property listings</Text>
      </YStack>

      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 120 }}>
        <YStack gap="$3">
          <YStack backgroundColor={panelBg} borderRadius={16} padding={14} borderWidth={1} borderColor={border} gap="$2">
            <Text color={titleColor} fontWeight="900" fontSize={14}>Quick Actions</Text>
          ...
          <YStack backgroundColor={panelBg} borderRadius={16} padding={14} borderWidth={1} borderColor={border} gap="$2">
            <XStack justifyContent="space-between" alignItems="center">
              <Text color={titleColor} fontWeight="900" fontSize={14}>Customer Bookings</Text>
            ...
            {!bookings.length && !bookingsLoading ? (
              <Text color={muted} fontSize={12}>No booking requests yet.</Text>
            ) : null}
            {bookings.map((pb) => {
              ...
              return (
                <YStack key={pb.id} backgroundColor={theme.bgCardSecondary} borderRadius={14} padding={12} gap="$2" borderWidth={1} borderColor={border}>
                  <XStack justifyContent="space-between" alignItems="center" flexWrap="wrap" gap="$2">
                    <YStack flex={1} gap={4}>
                      <Text color={titleColor} fontWeight="700" fontSize={13}>{prop?.title ?? 'Property'}</Text>
                      <Text color={muted} fontSize={11}>{[prop?.locality, prop?.city].filter(Boolean).join(', ') || '—'}</Text>
                      {pb.contact_name ? <Text color={muted} fontSize={11}>From: {pb.contact_name}</Text> : null}
                    </YStack>
                    <Text color={statusColor} fontSize={11} fontWeight="700" textTransform="uppercase">{pb.status}</Text>
                  </XStack>
                  <XStack gap="$2" flexWrap="wrap">
                    {pb.status === 'pending' ? (
                      <>
          ...
          <YStack backgroundColor={panelBg} borderRadius={16} padding={14} borderWidth={1} borderColor={border} gap="$2">
            <Text color={titleColor} fontWeight="900" fontSize={14}>Tips</Text>
            <Text color={muted} fontSize={12}>Keep photos clear and update pricing before publishing.</Text>
          </YStack>
        </YStack>
      </ScrollView>
    </View>
