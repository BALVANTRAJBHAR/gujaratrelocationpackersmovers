...
function buildFareTable(breakdown: Record<string, any> | null, estimated: number | null): string {
  ...
  for (const [key, val] of Object.entries(breakdown)) {
    ...
    if (key === 'total') {
      rows.push(`<tr style="border-top:2px solid #e2e8f0;"><td style="padding:10px 0 4px 0;font-weight:800;color:#0f172a;">${label}</td><td style="padding:10px 0 4px 0;text-align:right;font-weight:800;color:#0f172a;font-size:16px;">${display}</td></tr>`);
    } else if (key === 'subtotal' || key === 'gst') {
      rows.push(`<tr><td style="padding:4px 0;color:#475569;">${label}</td><td style="padding:4px 0;text-align:right;color:#475569;">${display}</td></tr>`);
    } else if (key !== 'vehicle_type_id') {
      rows.push(`<tr><td style="padding:4px 0;color:#64748b;font-size:13px;">${label}</td><td style="padding:4px 0;text-align:right;color:#64748b;font-size:13px;">${display}</td></tr>`);
    }
  }
  if (!rows.length) {
    return `<tr><td style="padding:8px 0;color:#334155;">Total Estimated</td><td style="padding:8px 0;text-align:right;font-weight:700;color:#0f172a;">${fmtCurrency(estimated)}</td></tr>`;
...
export async function generateBookingPdf(data: BookingPdfData, logoBase64?: string | null): Promise<string | null> {
  try {
    const reportId = `RPT-${String(data.id).slice(0, 8).toUpperCase()}`;
    ...
    <div class="section-body">
      <div class="info-grid">
        ...
        <div class="row">
          ...
          <span class="value"><span class="status-badge status-${escapeHtml(String(data.payment_status ?? 'pending'))}">${escapeHtml(String(data.payment_status ?? 'pending').replace('_', ' '))}</span></span>
        </div>
      </div>
    </div>
  </div>
    ...
    <div class="section-body" style="padding:10px 14px;">
      <table class="fare">
        ...
        <tr style="border-top:1px solid #cbd5e1;">
          <td style="padding:10px 0 4px 0;font-weight:800;color:#0f172a;">Advance Paid</td>
          <td style="padding:10px 0 4px 0;text-align:right;font-weight:700;color:#16a34a;">- ${fmtCurrency(data.advance_amount)}</td>
        </tr>
        <tr>
          <td style="padding:4px 0 10px 0;font-weight:800;color:#0f172a;font-size:15px;">Remaining to Pay</td>
          <td style="padding:4px 0 10px 0;text-align:right;font-weight:800;color:#dc2626;font-size:15px;">${fmtCurrency(data.remaining_amount)}</td>
        </tr>
      </table>
    </div>
  </div>
...
export async function shareBookingPdf(data: BookingPdfData): Promise<boolean> {
  try {
    ...
    const reportId = `RPT-${String(data.id).slice(0, 8).toUpperCase()}`;
    const fileName = `Shifting_Booking_Report_${reportId}.pdf`;
